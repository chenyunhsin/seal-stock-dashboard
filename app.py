...
# 1. Allow users to adjust stock shares interactively
st.subheader("🛠️ 調整您的目前持股")
col1, col2, col3 = st.columns(3)

with col1:
    sh_878 = st.number_input("00878 (張)", value=10, min_value=0) * 1000
    sh_888 = st.number_input("00888 永豐ESG (張)", value=11, min_value=0) * 1000
with col2:
    sh_919 = st.number_input("00919 (張)", value=11, min_value=0) * 1000
    sh_2330 = st.number_input("台積電 (零股)", value=3, min_value=0)
with col3:
    sh_2454 = st.number_input("聯發科 (零股)", value=1, min_value=0)
...
# 2. Fetch real-time stock prices
@st.cache_data
...
# 3. Display summary and table
st.subheader("📊 資產市值統計")
df_display = df_portfolio[["Name", "Shares", "Current_Price", "Total_Value"]].copy()
df_display.columns = ["股票名稱", "持有股數", "即時股價", "總市值"]
st.dataframe(df_display, use_container_width=True)

total_asset = df_portfolio["Total_Value"].sum()
st.metric(label="總股票市值 (TWD)", value=f"${total_asset:,.0f}")

# 4. AI-like insights and interaction feedback
...
if total_asset > 0:
    ...
    if etf_ratio > 85:
        st.markdown("✅ **建議方向**：你的組合高度集中在高股息 ETF（00878、00888、00919），這能帶來非常穩定的季配息現金流，適合作為防守型配置。")
    else:
        st.markdown("⚠️ **建議方向**：你的個股比例相對較高，雖然成長潛力大，但波動也會隨之增加。")
else:
